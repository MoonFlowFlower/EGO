...
| R-05 | `EgoCore/app/openemotion_adapter/event_builder.py` / `proto_self_adapter.py` / `result_consumer.py` / `proto_self_contract_validator.py` | schema adapter / kernel bridge | Dual | event/result contracts | E3 | 已接入 | 已启用 | R | adapter 中仍有边界漂移风险，需 schema-first 收口 | OpenEmotion contracts / runtime_v2 | 高 | 有 mirror | contract guard | 第二实现轮后半 | `openemotion_adapter/schemas + trace_bridge + restore_injector` | 新 contract E3/E4 后可绞杀旧隐式协议 | Dual |
...
| D-05 | 未登记 shim / compat helper | 未治理 shim | Dual | SHIM register | E0 | 未接入 | 未启用 | D | 长期会长成黑箱 | adapter / runtime / contracts | 高 | 未登记 | 无 | 进入删除池 | 登记后保留或删除 | 完成 shim inventory 后 | Dual |
...
- 本轮不删除任何 D 项。
- 第一实现轮只允许动 `R-01 / R-02 / R-06` 的最小骨架。
- `K-01 ~ K-04` 是观测与判真的基线，不进大清理。
