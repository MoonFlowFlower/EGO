...
> 目标：把任务单里的“建议目录结构”映射到当前仓库真实路径，供第一实现轮直接开工。
...
| `response_contract.response_plan` | `EgoCore/app/telegram_runtime_bridge.py` / `EgoCore/app/runtime_v2/runtime_reply.py` / `EgoCore/app/runtime_v2/proto_self_runtime.py` | EgoCore | 建立 ResponsePlan authority，不再让 verbalizer 承担 contract 决策 | 不替换全部 verbalizer 文案 | final / ask / wait / blocked 统一由 plan 驱动 |
...
| `openemotion_adapter.schemas.event_v1` | `EgoCore/app/openemotion_adapter/event_builder.py` + `OpenEmotion/openemotion/contracts/event_v1.py` | EgoCore authority / OpenEmotion mirror | 把 `event_v1` authority 收回 EgoCore 侧逻辑定义，OE 只 mirror | 不新建 shared/common 契约层 | adapter 输入 schema 唯一、可审计 |
...
| `OpenEmotion.service.process_event` | `OpenEmotion/openemotion/proto_self/kernel.py` / `OpenEmotion/openemotion/proto_self_v2/kernel.py` | OpenEmotion | 继续由主体本体处理 event | 不把 runtime 裁决迁入 OE | OE 先给结构化倾向，宿主后裁决 |

## 第一实现轮建议顺序

1. `interaction.classify_interaction`
2. `response_contract.response_plan`
3. `response_contract.memory_claim_gate`
4. 三者对应的最小 Telegram/testbot 回归

## 第二实现轮建议顺序

1. `tools.delivery_bridge`
2. `runtime.session_runtime` / `runtime.task_runtime` 明确拆层
3. `openemotion_adapter.schemas.event_v1` schema-first 收口
