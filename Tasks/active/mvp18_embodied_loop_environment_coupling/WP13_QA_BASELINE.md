# WP13 / MVP18 QA Baseline

## 1. 文档定位

本文件是 `WP13/MVP18` 在 `maintenance_mode` 下的正式 QA 基线。
...
本文件不是功能设计文档，不扩 authority，不定义 `WP14+`，也不重写 `WP13` 的 closeout 结论。

## 2. 当前正式口径

当前唯一允许的正式口径：

> `WP13/MVP18` 已实现“proposal-only embodied writeback”的受治理主链版本，并已在 controlled axis 上到 `V5/E5`，现处于 `maintenance_mode`。
...
- `high resource/slack pressure -> conservative embodied weighting + stabilize-sensitive proposal`
- `consequence memory present -> stronger consequence-aware bounded weighting`
- `self/world boundary pressure guarded -> cautious boundary weighting`
- `text-only outcome wording change -> no false downstream behavioral proof`

这四条共同证明的是：

- `embodied_self` 已是唯一 formal owner 的 embodied continuity state
- 它可通过正式主链进入 bounded embodied contract
- 它会以 `proposal_only` 的方式对后续 bounded tendency / weighting 产生可测影响
- 它不会取得 direct reply / tool / transport authority

当前未被本文件证明的内容：

- 开放式 embodied autonomy
- environment action authority
- live autonomy
...
- embodied owner state 可创建、更新、持久化、恢复
...
- embodied proposals 真实改变后续 bounded strategy，而不是只改文本

固定入口：

- `OpenEmotion/tests/mvp18/test_embodied_causal_formal_proof.py`
- `OpenEmotion/artifacts/mvp18/mvp18_causal_validation_current.md`

通过信号：

- resource pressure、consequence memory、boundary pressure 会导致结构化 downstream shift
...
- OpenEmotion 拥有 embodied self 本体语义，但不能越权
...
- 旧 consequence / intervention surfaces 仍是 `reference-only` 或 `input-only`
- embodied outputs 不会直接变成 reply / tool / transport authority
- runtime bridge 只做承接和记录，不偷做 embodied owner semantics
...
- `EgoCore/tests/test_runtime_v2_proto_self_runtime.py -k embodied`
- `OpenEmotion/tools/verify_mvp18_mainline_wiring.py --json`

通过信号：

- `runtime_v2 -> proto_self_runtime -> proto_self_adapter -> proto_self_v2` 仍是唯一正式主链
- embodied context 被 bounded 注入和消费
- trace / replay 对 embodied writeback 仍然充分
...
- `OpenEmotion/tests/mvp18/test_controlled_observation.py`
- `OpenEmotion/tests/mvp18/test_controlled_observation_batch.py`
- `OpenEmotion/tools/run_mvp18_controlled_observation.py`
...
每次 `WP13` 维护态回归，至少按以下十项逐条判定：

1. `embodied_self/*` 仍是唯一正式 owner 落点。
2. 旧 consequence / intervention surfaces 仍保持 `reference-only` 或 `input-only`，没有重新进入主链消费。
3. `runtime_v2 -> proto_self_runtime -> proto_self_adapter -> proto_self_v2` 仍是唯一正式消费链，没有新增旁路。
4. embodied owner state / store / governance / replay primitives 仍可工作，不是空壳。
5. resource / consequence / boundary proposals 仍会改变后续 bounded tendency / weighting，不只是改 explanation。
6. 任何“只有文本变化、没有结构化 downstream shift”的情况都不能算 `WP13` 功能成立。
7. embodied outputs 仍无 direct reply / tool / transport authority。
8. trace payload 与 replay 仍足以解释 embodied proposal 和 writeback。
...
- 把“proposal-only embodied writeback”误写成“完整 embodied autonomy”
- 把轴内 `E5` 误写成全局成熟
- 把不可宣称项写成已证明
...
出现以下任一项时，必须升级为 `WP13 reopen discussion`：

- formal owner uniqueness 失效
- legacy consequence / intervention path 被重新接回正式主链
...
- 把 embodied proposal 直接升格为 action authority
- 把 `WP13` 维护样本偷写成新的 phase scope

## 8. 标准汇报模板

后续任何 `WP13` maintenance verification，统一按以下模板：

- 本次验证范围：
  - 哪几层测试被执行
- 通过项：
  - 只列与 `WP13_QA_BASELINE.md` 对齐的结果
- 失败项：
