...
def inspect_wiring() -> dict:
    ...
    mainline_writeback_consumer_present = any(
        [
    ...
    if core_uses_reflection_shadow and not mainline_writeback_consumer_present:
        status = "shadow_only_mainline_writeback_missing"
    elif mainline_writeback_consumer_present:
        status = "mainline_writeback_consumer_present"
    else:
        status = "reflection_mainline_not_detected"

    return {
        "schema_version": "mvp15.mainline_wiring_check.v1",
        "root": str(ROOT),
        "formal_owner": {
        ...
        "mainline": {
            "writeback_consumer_present": mainline_writeback_consumer_present,
        },
        "status": status,
    }
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"mainline.writeback_consumer_present: {report['mainline']['writeback_consumer_present']}")

    if args.require_writeback_consumer and not report["mainline"]["writeback_consumer_present"]:
        return 1
    return 0
