...
async def test_trust_persists_after_betrayal(isolated_db):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        await client.post("/event", json={"type": "world_event", "actor": "user_B", "target": "assistant", "meta": {"subtype": "betrayal"}})
        await client.post("/event", json={"type": "world_event", "actor": "user_B", "target": "assistant", "meta": {"subtype": "repair_success"}})
...
async def test_repair_bank_repair_success(isolated_db):
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        await client.post("/event", json={"type": "world_event", "actor": "user_A", "target": "assistant", "meta": {"subtype": "betrayal"}})
        r1 = await client.post("/plan", json={"user_id": "user_A", "user_text": "test"})
        repair_bank_before = r1.json()["relationship"]["repair_bank"]
        await client.post("/event", json={"type": "world_event", "actor": "user_A", "target": "assistant", "meta": {"subtype": "repair_success"}})
