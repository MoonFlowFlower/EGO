...
def git_dirty_paths(*, tracked_only: bool = True, repo_root: Path = ROOT) -> List[str]:
    ...
    output = git_output(args, repo_root=repo_root)
    rows: List[str] = []
...
def classify_tracked_dirty_paths(paths: List[str]) -> Dict[str, List[str]]:
    allowed_runtime = [path for path in paths if path in ALLOWED_RUNTIME_DIRTY_PATHS]
    unexpected = [path for path in paths if path not in ALLOWED_RUNTIME_DIRTY_PATHS]
    return {
