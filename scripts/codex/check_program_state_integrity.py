...
def main() -> int:
    ...
    if declared_generated != expected_generated:
        errors.append("integrity.generated_views must exactly enumerate the generated status outputs")
    if integrity.get("compatibility_mirrors") not in (None, []):
        errors.append("integrity.compatibility_mirrors must be empty after legacy archival purge")
    if integrity.get("legacy_archive_pointer") != repo_relative(LEGACY_ARCHIVE_POINTER_PATH):
        errors.append("integrity.legacy_archive_pointer must point at legacy/ego-pre-handmade-mainline/ARCHIVED_POINTER.md")

    for path in [ROOT_README, LEGACY_ARCHIVE_POINTER_PATH, CURRENT_LOGIC_FLOW]:
        if not path.exists():
