#!/usr/bin/env python3
"""Callable development-context gate for hierarchical outcome-delta repair.
...
TASK_ID = "EGO-V2-P1-HIERARCHICAL-OUTCOME-DELTA-REPAIR-001C-R2"
...
CLAIM_CEILING = (
    "Replayable identified additive outcome-delta implementation and measured "
    "improvement or failure on already-consumed worlds 52/54 with policy seed "
    "711 only."
)
PRODUCER = "verify_ego_v2_hierarchical_outcome_delta_repair_001c_r2"
...
def run_boundary(output_dir: Path) -> dict[str, Any]:
    ...
    smoke = _provenance(
        ...
        "schema_version": "ego.v2.hierarchical_outcome_delta_repair.smoke.v1",
        "runs": runs,
        "checks": checks,
                ...
                for candidate in engine.ACTIONS:
                    ...
                    rows.append(
                        ...
                        | {
                            ...
                            "sequence": sequence,
                            "action": candidate,
                            "outcome_type": str(truth["outcome_type"]),
...
def _row_delta_mae(row: Mapping[str, Any]) -> float:
    if "delta_mae" in row:
        return float(row["delta_mae"])
    return float(row["scores"]["delta_mae"])


def _stratified_delta(rows: list[Mapping[str, Any]]) -> dict[str, Any]:
    strata: dict[str, Any] = {}
    estimable_values: list[float] = []
    for action, outcome in REALIZABLE_STRATA:
        selected = [
            ...
            and str(
                row.get("outcome_type", (row.get("truth") or {}).get("outcome_type"))
            )
            == outcome
        ]
        support = len(selected)
        estimable = support >= MIN_STRATUM_SUPPORT
        mae = (
            statistics.fmean(_row_delta_mae(row) for row in selected)
            if selected
            else None
        )
        ...
        if estimable and mae is not None:
            estimable_values.append(mae)
    all_estimable = len(estimable_values) == len(REALIZABLE_STRATA)
    return {
        "minimum_support": MIN_STRATUM_SUPPORT,
        "strata": strata,
        "all_strata_estimable": all_estimable,
        "macro_delta_mae": (
            statistics.fmean(estimable_values) if all_estimable else None
        ),
        "aggregation_rule": (
...
def _training_support_and_rank(
    output_dir: Path, smoke: Mapping[str, Any]
) -> dict[str, Any]:
    counts: Counter[tuple[str, str]] = Counter()
    features_by_action: dict[str, list[np.ndarray]] = defaultdict(list)
    for run, (layout, world_seed, policy_seed) in zip(smoke["runs"], CONTEXTS):
        del layout, world_seed, policy_seed
        with SQLiteEventStore(output_dir / run["database_path"]) as store:
            recovered = store.recover_run(run["run_id"])
        for previous_frame, frame in zip(recovered.frames, recovered.frames[1:]):
            trace = frame.trace
            if trace is None or trace.get("selected_action") is None:
                continue
            action = str(trace["selected_action"])
            outcome = str(trace["world_transition"]["outcome_type"])
            ...
            features_by_action[action].append(
                predictive_control._feature_vector_from_summary(  # noqa: SLF001
                    organism=payload["organism"],
                    summary=payload["belief_summary"],
                )
            )
    ...
    ranks = {
        action: (
            int(np.linalg.matrix_rank(np.stack(features))) if features else 0
        )
        for action, features in features_by_action.items()
    }
    for action in engine.ACTIONS:
        ranks.setdefault(action, 0)
    return {
        "minimum_support": MIN_STRATUM_SUPPORT,
        "stratum_support": stratum_support,
        "all_strata_supported": all(
            count >= MIN_STRATUM_SUPPORT for count in stratum_support.values()
        ),
        "feature_count": len(predictive_control.FEATURE_NAMES),
        "feature_rank_by_action": ranks,
        "all_action_feature_ranks_full": all(
            rank == len(predictive_control.FEATURE_NAMES) for rank in ranks.values()
        ),
    }


def _model_ablation_rows(
    learned_rows: list[Mapping[str, Any]],
    snapshots: list[Mapping[str, Any]],
    *,
    mode: str,
) -> list[dict[str, Any]]:
    if mode not in {"base_only", "residual_only", "residual_outcome_rotation"}:
        raise ValueError("unknown hierarchical delta ablation")
    snapshot_by_hash = {
        str(snapshot["snapshot_hash"]): snapshot for snapshot in snapshots
    }
    rows: list[dict[str, Any]] = []
    for learned in learned_rows:
        snapshot = snapshot_by_hash[str(learned["snapshot_hash"])]
        ablated_state = deepcopy(snapshot["predictive_state"])
        model = ablated_state["model"]
        if mode == "base_only":
            model["delta_outcome_offsets"] = predictive_control._empty_delta_outcome_offsets()  # noqa: SLF001
        elif mode == "residual_only":
            model["delta_base_weights"] = predictive_control._empty_delta_base_weights()  # noqa: SLF001
        else:
            offsets = np.asarray(
                model["delta_outcome_offsets"],
                dtype=predictive_control.NUMERIC_DTYPE,
            )
            model["delta_outcome_offsets"] = np.roll(offsets, shift=1, axis=1).tolist()
        ...
        rows.append(
            _provenance(
                "_model_ablation_rows",
                inputs=[
                    f"snapshot:{learned['snapshot_hash']}",
                    f"ablated_model:{_canonical_hash(model)}",
                    f"truth:{_canonical_hash(truth)}",
                ],
                run_id=str(learned["run_id"]),
                aggregation_rule=f"callable_{mode}_prediction_on_same_balanced_snapshot",
...
def run_balanced(output_dir: Path, smoke: Mapping[str, Any]) -> dict[str, Any]:
    ...
    ablation_rows = {
        mode: _model_ablation_rows(learned_rows, snapshots, mode=mode)
    ...
    training_support = _training_support_and_rank(output_dir, smoke)
    stratified = {
        model: {
            phase: _stratified_delta(
                [
    ...
    legacy_late_macro = stratified["legacy_unconditional"]["late"]["macro_delta_mae"]
    all_effect_strata_estimable = all(
        report[phase]["all_strata_estimable"]
        for report in stratified.values()
        for phase in ("early", "late")
    )

    def _macro_below(left: float | None, right: float | None) -> bool:
        return left is not None and right is not None and left < right

    def _critical_strata_not_worse(reference: str) -> bool:
    ...
    checks = {
        ...
        "training_support_at_least_16_per_declared_stratum": training_support[
            "all_strata_supported"
        ],
        "shared_base_feature_rank_is_full_for_every_action": training_support[
            "all_action_feature_ranks_full"
        ],
        "all_effect_strata_have_at_least_16_rows_per_phase": all_effect_strata_estimable,
        "learned_late_macro_delta_mae_below_early": _macro_below(
            learned_late_macro, learned_early_macro
        ),
        "learned_late_macro_delta_mae_below_no_update_late": _macro_below(
            learned_late_macro, no_update_late_macro
        ),
        "learned_late_macro_delta_mae_below_legacy_unconditional": _macro_below(
            learned_late_macro, legacy_late_macro
        ),
        "interact_strata_not_worse_than_no_update": _critical_strata_not_worse(
            "no_update"
        ),
        "interact_strata_not_worse_than_legacy_unconditional": _critical_strata_not_worse(
            "legacy_unconditional"
        ),
        "base_only_ablation_changes_late_macro_delta": (
            learned_late_macro is not None
            and ablation["base_only"]["late"]["macro_delta_mae"] is not None
            and not math.isclose(
                learned_late_macro,
                ablation["base_only"]["late"]["macro_delta_mae"],
                abs_tol=1e-15,
            )
        ),
        "residual_only_ablation_changes_late_macro_delta": (
            learned_late_macro is not None
            and ablation["residual_only"]["late"]["macro_delta_mae"] is not None
            and not math.isclose(
                learned_late_macro,
                ablation["residual_only"]["late"]["macro_delta_mae"],
                abs_tol=1e-15,
            )
        ),
        "outcome_rotation_changes_late_macro_delta": (
            learned_late_macro is not None
            and ablation["residual_outcome_rotation"]["late"]["macro_delta_mae"]
            is not None
            and not math.isclose(
                learned_late_macro,
                ablation["residual_outcome_rotation"]["late"]["macro_delta_mae"],
                abs_tol=1e-15,
            )
        ),
        "forced_action_truth_exists_for_every_balanced_row": all(
            str(row["truth"]["outcome_type"]) in predictive_control.OUTCOMES
            for row in rows
        ),
    ...
    report = _provenance(
        ...
        "schema_version": "ego.v2.hierarchical_outcome_delta_repair.balanced.v1",
        ...
        "training_support_and_rank": training_support,
        "legacy_unconditional_metrics": legacy,
        "hierarchical_ablation_metrics": ablation,
        "rows": rows,
...
def run_gate(output_dir: Path) -> dict[str, Any]:
    ...
    if smoke["boundary_passed"]:
        ...
        if balanced["passed"]:
            verdict = "HIERARCHICAL_DELTA_REPAIRED_ON_DEVELOPMENT_CONTEXTS"
        elif any(
            name in balanced["failed_checks"]
            for name in (
            ...
            verdict = "HIERARCHICAL_DELTA_REPAIRED_NO_DELTA_IMPROVEMENT"
    else:
    ...
    baseline = _provenance(
        ...
        "outcome_stratified": balanced.get("outcome_stratified_metrics"),
        "training_support_and_rank": balanced.get("training_support_and_rank"),
    ...
    ablation = _provenance(
        ...
        aggregation_rule="callable_base_only_residual_only_and_outcome_rotation_on_same_balanced_snapshots",
        context_ids=[_context_id(*context) for context in CONTEXTS],
        life_ids=[1, 4],
        action_ids=list(engine.ACTIONS),
    ) | {
        "hierarchical_ablations": balanced.get("hierarchical_ablation_metrics"),
        "status": "completed" if smoke["boundary_passed"] else "not_run_boundary_failed",
    }
    ...
    eligibility = _provenance(
        ...
        aggregation_rule="all_development_gates_required_before_separate_effect_card_eligibility",
        context_ids=[_context_id(*context) for context in CONTEXTS],
        action_ids=list(engine.ACTIONS),
    ) | {
        "fresh_effect_seeds_consumed": False,
        "eligible_for_separate_effect_card": balanced.get("passed", False),
        "verdict": verdict,
    }
    result = _provenance(
        "run_gate",
        ...
        _canonical_json(
            {
                ...
                "fresh_effect_seeds_consumed": False,
                "eligible_for_separate_effect_card": result[
                    "eligible_for_separate_effect_card"
                ],
            }
        )
    )
