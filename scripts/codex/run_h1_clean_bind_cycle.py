...
def _wait_for_live_bind(
    *,
    ...
    while time.time() < deadline:
        if live_path.exists():
            last_payload = json.loads(live_path.read_text(encoding="utf-8"))
    ...
    return {
        ...
        "launcher_meta_payload": json.loads(launcher_meta.read_text(encoding="utf-8")) if launcher_meta.exists() else {},
    }
