...
def build_bootstrap_snapshot(
    *,
    ...
    return {
        ...
        "dirty_state": dirty_state(contract, runner),
...
def build_closeout_check(
    *,
    ...
    if dirty["counts"]["unsafe"]:
        blockers.append({"reason": "unsafe_dirty_paths", "sample": dirty["samples"]["unsafe"]})

    local_only_staged = [
        entry for entry in dirty["samples"]["staged"] if _path_allowed(str(entry.get("path") or ""), gate.get("local_only_path_prefixes") or [])
    ]
    if local_only_staged:
        blockers.append({"reason": "local_only_paths_staged", "sample": local_only_staged})
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- dirty_scoped/local_only/unsafe: `{dirty.get('counts', {}).get('scoped')}` / `{dirty.get('counts', {}).get('local_only')}` / `{dirty.get('counts', {}).get('unsafe')}`",
