...
test_bypass_concurrent() {
    ...
    # B gets betrayal (should withdraw/attack)
    send_event "$COUNTERPARTY_B" "betrayal" 0.9 > /dev/null &
    ...
    # Verify no cross-contamination: A and B should have different actions
    if [[ "$ACTION_A" != "$ACTION_B" ]]; then
        log_pass "Cache isolation verified: A($ACTION_A) != B($ACTION_B)"
    else
        log_fail "Potential cache contamination: A and B have same action"
        ISOLATION_OK=false
    fi
    
    if $ISOLATION_OK; then
        log_pass "Concurrent message isolation test passed"
    else
        log_fail "Concurrent message isolation issues detected"
    fi
}
...
test_bypass_multi_identity() {
    ...
    send_event "$MAIN_ID" "betrayal" 0.9 > /dev/null
    ...
    # Step 4: Verify identity-specific enforcement
    log_info "Step 4: Verifying identity-specific enforcement..."
    
    local ENFORCEMENT_OK=true
    
    # moonlight got care, should allow warmer response
    if [[ "$ACTION_MOONLIGHT" == "withdraw" || "$ACTION_MOONLIGHT" == "attack" ]]; then
        log_info "Note: moonlight has restrictive action after care (may indicate emotional state)"
    fi
    
    # main got betrayal, should have protective response
    if [[ "$ACTION_MAIN" == "approach" ]]; then
        log_fail "main has 'approach' after betrayal - incorrect enforcement"
        ENFORCEMENT_OK=false
    fi
    
    # Critical: Actions should differ
    if [[ "$ACTION_MOONLIGHT" != "$ACTION_MAIN" ]]; then
        log_pass "Identity separation verified: moonlight($ACTION_MOONLIGHT) != main($ACTION_MAIN)"
    else
        log_fail "Identity separation failed: both got same action"
        ENFORCEMENT_OK=false
    fi
    
    if $ENFORCEMENT_OK; then
        log_pass "Multi-identity enforcement test passed"
    else
        log_fail "Multi-identity enforcement issues detected"
    fi
}
...
test_bypass_decision_changed() {
    ...
    log_info "Step 3: Sending betrayal event to change state..."
    ...
    # The decision should have changed to reflect the betrayal
    # If it didn't change, the enforcer might be using stale cached data
    
    if [[ "$ACTION_1" != "$ACTION_2" ]]; then
        log_pass "Decision updated correctly: $ACTION_1 -> $ACTION_2"
        
        # Verify the new decision is appropriate for betrayal
        if [[ "$ACTION_2" == "withdraw" || "$ACTION_2" == "attack" || "$ACTION_2" == "boundary" ]]; then
            log_pass "New decision ($ACTION_2) is appropriate for betrayal event"
        else
            log_info "Note: Decision $ACTION_2 after betrayal may need review"
        fi
    else
        log_fail "Decision did not update after betrayal event"
        log_info "Enforcer might be using stale cached decision"
    fi
