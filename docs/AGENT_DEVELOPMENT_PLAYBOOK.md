...
> 2026-05-18 transition note: 当前默认 human/operator 体验主线已切到 `EgoOperator`。本 playbook 只用于读取或维护 `legacy/ego-pre-handmade-mainline/EgoCore` 与 `legacy/ego-pre-handmade-mainline/OpenEmotion` 的历史实现，不再是新任务默认入口。新任务优先看 `docs/MAINLINE_QUICKSTART.md` 与 `EgoOperator/`。
