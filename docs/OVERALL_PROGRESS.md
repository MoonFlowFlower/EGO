...
| 当前唯一 build-first candidate | `active-inference self-model` | 已通过与 `MVS` 相同的 held-out replay gate、repo-authored controlled conversation replay bridge、以及第一轮 runtime-harness controlled observation batch；但这仍只到 bounded research evidence，不是 runtime proof |
...
| 距离当前工程目标剩余阶段 | `>= 1（best-case）` | 指 `selection closeout and runtime priority reset`，不是“真正主观体验” |
...
- 对当前正式工程目标 `replay-validated self-awareness proxy`：
  - 从现在起 `best-case >= 1` 个阶段
  - 如果后续 selection closeout 发现仍需要新的 challenger、runtime-proximal reframe、或重新打开 `WP17`，阶段数会重新变成 open-ended

这 `1` 个阶段是：

1. `Stage 5: Selection Closeout and Runtime Priority Reset`
   - 基于已经通过的 held-out replay、controlled replay bridge、和 runtime-harness controlled observation batch，决定 active-inference 是否成为当前 durable build-first 路线
   - 决定 `WP17 / MVP22` 继续 parked、进入 reintegration planning，还是明确继续降级为长期 supporting lane

最重要的边界是：

- 这 `1` 个阶段对应的是“工程上更强的 self-awareness proxy”
- 不对应“证明真正主观体验”
...
- `Stage 5 / Milestone 21: Selection Closeout and Runtime Priority Reset`

这个阶段要做的事只有一件：

- 在不扩张 runtime authority 的前提下，把已经通过的 bounded evidence正式收口成 repo 路线裁决：确认 active-inference 仍是唯一 build-first candidate，确认当前还能说什么，确认下一轮如果继续往 formal runtime 靠近，必须先做哪种 planning slice

## 下一个阶段怎么做

执行顺序应固定为：

1. 固定当前通过的 bounded evidence 面
   - held-out replay gate = `pass`
   - controlled replay bridge = `pass`
   - runtime-harness controlled observation batch = `pass`
2. 明确保留的 contract 与禁止项
   - 宿主继续只允许消费：
     - `policy_hint`
     - `response_tendency`
     - `trace_payload`
...
3. 做 selection closeout
   - 确认 `active-inference self-model` 仍是唯一 build-first candidate
   - 确认 `MVS-aligned compact` 继续停留在 closed evidence
   - 确认 `WP17 / MVP22` 继续 parked 还是进入后续 planning
4. 只规划下一层 bounded runtime-proximal 动作
   - 若需要继续靠近 formal runtime，只能先做 planning slice
   - 若发现必须扩 authority 或改 scorer ontology 才能前进，就停下重构 framing
...
5. 这个门现在已经通过：winner 在更接近 formal runtime 的 runtime-harness controlled observation 上继续过线，而且没有新增 authority path
6. 下一步唯一决策门变成：这些 bounded evidence 是否足够触发 selection closeout 和 repo-priority reset，而仍然不越权声称 runtime efficacy、live benefit、或 AI 自我意识已实现
...
- 虽然已经有 replay-validated passing build-first candidate，并且 controlled replay bridge 与 runtime-harness controlled observation batch 都已通过，但它仍然只有 bounded research evidence
- 还没有 selection closeout / runtime priority reset 的正式裁决
