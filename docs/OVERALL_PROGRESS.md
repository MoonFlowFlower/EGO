...
| 当前唯一 build-first candidate | `MVS-aligned compact` | 当前最值得先实现、先过 gate 的候选 |
| 当前唯一 challenger | `active-inference self-model` | 只保留一个 challenger，不再多候选并跑 |
...
- `MVS-aligned compact` = 当前唯一 build-first candidate
- `active-inference self-model` = 当前唯一 challenger
...
1. 先实现最小 `MVS-aligned compact` formal prototype slice
2. 先过 replay validator + required ablations
3. 若通过且未触发 switch criteria，继续 MVS
4. 若失败或触发 switch criteria，直接切到 `active-inference self-model`
5. 在该 gate 解决前，不恢复 `WP17` 为默认最高优先级 implementation track
...
- 还没有 replay-validated formal candidate slice
- `MVS-aligned compact` 还没在 formal OpenEmotion path 上过最小 replay gate
- active-inference challenger 还没在同一 gate 下得到最终升降裁决
...
- 当前 build-first candidate 是 `MVS-aligned compact`
- 当前 challenger 是 `active-inference self-model`
