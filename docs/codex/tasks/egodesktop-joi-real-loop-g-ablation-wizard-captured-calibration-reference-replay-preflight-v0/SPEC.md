...
- owner: `Codex`
- layer: `engineering validation / captured calibration reference replay preflight`
...
Execute the separate calibration/reference slice after 027. This rebuilds a Wizard-specific captured calibration
reference by consuming the accepted 009 predeclared calibration row and checking it against the accepted 026 Wizard
heldout row, then rebuilds the Wizard `OFF_STATIC_REPLAY_HELDOUT` row with that captured reference and reruns replay
preflight. This task emits 028-specific artifacts but does not score, compare, attribute, enable runtime paths, or
advance the route.
...
- claim ceiling: `wizard_captured_calibration_reference_replay_preflight_only`.
- minimal validation: callable builder/evaluator reports, raw-text field scan, route convergence, repo fast verifier,
  diff check, scoped closeout.
- stop condition: raw text staging, old 009 reference reuse as a Wizard-specific pass without a new 028 partition
  report, scoring/comparison/verdict, default runtime enablement, program-state/evidence-ledger update, push, tag, or
  remote anchor.
- rollback plan: delete this task package, remove `EGODESKTOP-GABLATION-028` from the task board, and regenerate route
  views.
- acceptance signal: a committed task package, 028-specific captured reference/replay/eval artifacts, task-board entry,
  and verification readback that cleanly separates captured-reference preflight from 027 and preserves all claim
  ceilings.
...
- problem definition: 027 proved that the 026 Wizard heldout row can pass `OFF_STATIC_REPLAY_HELDOUT` structure with a
  ...
  provenance.
- current stage/layer: `engineering validation / captured calibration reference replay preflight`.
- mainline target: explicit offline artifact scripts only, not default runtime.
- enabled-state requirement: explicit local Node commands only, not runtime enablement.
- real-trigger evidence requirement:
  ...
  - partition/reference output must be 028-specific and must not reuse the old 009 partition as final Wizard evidence.
...
- acceptance gate:
  ...
  - a new 028 calibration reference is built using 009 calibration rows and 026 heldout rows;
  - Wizard heldout OFF_STATIC is rebuilt with the new 028 calibration reference;
  - replay preflight evaluation reports scoring/verdict disabled;
  - raw text scan reports no selected source or calibration utterance leakage fields;
  - route/repo checks and scoped closeout pass.
- claim ceiling: this proves only Wizard captured calibration reference replay preflight, not route success.
...
Only that current callable scripts produced a 028-specific captured calibration reference, a rebuilt Wizard
`OFF_STATIC_REPLAY_HELDOUT` row, and replay integrity preflight artifacts with scoring/verdict disabled.
