...
2. TDD RED for read-only first-89 extraction, current-source recovery equality,
   control-derived max-life/respawn bounds, lifecycle/budget accounting,
   quotient projection, and verdict priority; observe the intended failure.
...
5. TDD RED/GREEN for explicit independent-reset panel rollout IDs 9..16,
   ...
   truth, public-hash dedupe, panel support/rank, and model-state non-access.
