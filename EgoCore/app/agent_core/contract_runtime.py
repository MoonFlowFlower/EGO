...
def _derive_html_title(goal: str, artifact_text: str) -> str:
    text = artifact_text or goal or ""
    if re.search(r"egocore", text, flags=re.IGNORECASE):
        return "EgoCore"
    match = re.search(r"介绍\s*([A-Za-z][A-Za-z0-9_-]*)", text, flags=re.IGNORECASE)
    if match:
        return match.group(1)
    return "Landing Page"


def _render_fast_html(goal: str, artifact_text: str) -> str:
    title = _derive_html_title(goal, artifact_text)
    subtitle = artifact_text.strip() or goal.strip() or "Generated by EgoCore"
    ...
    return (
        ...
        "  <style>\n"
        "    :root { color-scheme: light; --bg:#f7f4ea; --ink:#111111; --accent:#e84a2a; --panel:#fff9ef; }\n"
        "    * { box-sizing:border-box; }\n"
        "    body { margin:0; font-family:\"Segoe UI\",\"PingFang SC\",\"Microsoft YaHei\",sans-serif; background:linear-gradient(135deg,#f7f4ea 0%,#efe1c6 45%,#f9f6ef 100%); color:var(--ink); }\n"
        "    main { min-height:100vh; display:grid; place-items:center; padding:32px; }\n"
        "    section { width:min(980px,100%); background:rgba(255,249,239,0.92); border:1px solid rgba(17,17,17,0.08); border-radius:28px; padding:40px; box-shadow:0 28px 80px rgba(17,17,17,0.12); }\n"
        "    .eyebrow { display:inline-block; padding:8px 14px; border-radius:999px; background:#111; color:#fff; font-size:12px; letter-spacing:0.16em; text-transform:uppercase; }\n"
        "    h1 { font-size:clamp(44px,8vw,96px); line-height:0.92; margin:18px 0 16px; letter-spacing:-0.05em; }\n"
        "    p { max-width:680px; font-size:18px; line-height:1.7; margin:0; }\n"
        "    .grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:18px; margin-top:32px; }\n"
        "    .card { background:var(--panel); border-radius:22px; padding:22px; border:1px solid rgba(17,17,17,0.08); }\n"
        "    .card strong { display:block; margin-bottom:10px; font-size:16px; }\n"
        ...
        "  <main>\n"
        "    <section>\n"
        "      <span class=\"eyebrow\">EgoCore Generated</span>\n"
        f"      <h1>{title_html}<span class=\"accent\">.</span></h1>\n"
        f"      <p>{subtitle_html}</p>\n"
        "      <div class=\"grid\">\n"
        "        <article class=\"card\"><strong>Intentional Host</strong><span>Telegram ingress, tool execution, runtime governance, and observable traces.</span></article>\n"
        "        <article class=\"card\"><strong>Native Mainline</strong><span>Contract lock, next-step execution, verification, and controlled fallback boundaries.</span></article>\n"
        "        <article class=\"card\"><strong>OpenEmotion Hooked</strong><span>Proto-self signals feed policy hints without taking over runtime ownership.</span></article>\n"
        "      </div>\n"
        "    </section>\n"
        "  </main>\n"
        "</body>\n"
...
class ContractRuntimeEngine:
    ...
    def should_fast_path_html_write(self, contract: TaskContract, ingress_context: Optional[Dict[str, Any]] = None) -> bool:
        ingress_context = ingress_context or {}
        artifact_text = ingress_context.get("resolved_artifact_text") or ""
        return bool(
            contract.target_path
            and contract.output_format == "html"
            and ingress_context.get("runtime_action") == "execute_task"
            and artifact_text
        )

    def execute_fast_html_write_step(
        self,
        *,
        contract: TaskContract,
        ingress_context: Optional[Dict[str, Any]] = None,
        ...
        ingress_context = ingress_context or {}
        content = _render_fast_html(contract.goal, ingress_context.get("resolved_artifact_text") or "")
        result = execute_tool(
            "file",
            {
                "operation": "write",
        ...
        return {
            "tool_name": "file",
            "arguments": {"operation": "write", "path": contract.target_path},
